...
| M3 | **Fixed (decided-by-Michael: REWRITE-HONEST).** All 11 "Download the Printable Worksheet Pack" CTAs rewritten ("A printable companion worksheet pack is in the works — see details. Until it arrives, this page works beautifully with your own notebook or journal."); `worksheet-download-fallback.xhtml` rewritten honestly with a michaeldavidjr.beauty notify pointer. Zero "Download the Printable" strings remain. (Phase 1's "~18 locations" resolved to exactly 11 CTA blocks + the fallback page in the EPUB sources.) |
...
2. **The honest worksheet copy promises a pack "in the works"** — that is now a public commitment in a permanent edition. Ship the MailerLite-gated pack (Phase 2 T3 deadline was June 16) or the promise itself ages badly.
...
4. **New unverifiable-claim candidates the audit did not flag** (all currently hedged/safe, listed for completeness): the Ch XIV claim that "more states are requiring texture competency for licensing" (true for several states, uncited); Ch XI's "DaySmart Salon" AI-features description (plausible vendor capability, uncited); and the Ch VII Jen Atkin wellness-case details, which rest on a single Hoffman Institute interview — all are soft claims, none names a private person, none is quote-marked.
