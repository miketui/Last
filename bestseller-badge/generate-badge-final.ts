/**
 * Premium 3D Metallic Gold Bestseller Badge with Custom Logo - FINAL
 * Logo on solid gold background, properly integrated
 */
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}">
  <defs>
    ...
    <!-- Center gold for logo background - solid gold -->
    <radialGradient id="logoBackground" cx="35%" cy="35%" r="65%" fx="35%" fy="35%">
  ...
  <g filter="url(#medalShadow)">
    ...
    <!-- Gold background for logo area - SOLID -->
    <circle cx="${centerX - 2}" cy="${centerY - 2}" r="205"
            fill="url(#logoBackground)"
            filter="url(#sharpBevel)"/>

    <!-- Solid gold circle behind logo to eliminate transparency -->
    <circle cx="${centerX - 2}" cy="${centerY - 2}" r="180"
            fill="#C49000"/>

    <!-- Inner ring around logo -->
    <circle cx="${centerX - 1}" cy="${centerY - 1}" r="182"
            fill="none"
            stroke="url(#goldSharp)"
            stroke-width="3"
            filter="url(#ultraEmboss)"/>

    <!-- Logo embedded in center with circular clip -->
...
await Bun.write('/tmp/images/bestseller-final.svg', svg);
console.log('Final SVG created: /tmp/images/bestseller-final.svg');
...
await Bun.write('/tmp/images/bestseller-final.png', pngBuffer);
console.log('Final PNG created: /tmp/images/bestseller-final.png');
